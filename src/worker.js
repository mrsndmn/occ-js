...
// set `MODEL_ID = "OCC-RAG-0.6B-ONNX"`, and add `env.allowRemoteModels = false`.
...
async function generate({ messages, documents }) {
  // Retrieve the text-generation pipeline.
  const [tokenizer, model] = await TextGenerationPipeline.getInstance();

  // OCC-RAG uses a custom chat template. When `documents` are supplied they are
  // attached to the final user turn as numbered sources for grounded answering.
  ...
  // The model's generation_config uses greedy decoding (do_sample=false).
